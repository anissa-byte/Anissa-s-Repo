...
This skill follows the **School of Bots Design System** defined in [`SOB-DESIGN-SYSTEM.md`](/Users/anissafulbright/Anissa's Repo/SOB-DESIGN-SYSTEM.md).

Key tokens used in the invoice PDF:

| Token | Hex | Usage in invoice |
|---|---|---|
| Navy | `#04005B` | Header bar, headlines, section labels, "Bill to" / meta labels |
| Ebony | `#0D0A1B` | Body text, table data |
| White Smoke | `#F8F9FA` | Table header row background |
| Texas Rose | `#FFBD4A` | Logo accent badge |
| Portage | `#A59FF7` | "Pay online" link color |

When updating the invoice template, always pull colors from the design system — never introduce ad-hoc hex values.
...
def draw_invoice(output_path, invoice_number, issue_date, due_date):
    ...
    # SOB Design System colors (see SOB-DESIGN-SYSTEM.md)
    NAVY = colors.HexColor("#04005B")
    TEXAS_ROSE = colors.HexColor("#FFBD4A")
    ...
    c.setFont("Helvetica", 9)
    PORTAGE = colors.HexColor("#A59FF7")  # SOB Design System
    c.setFillColor(PORTAGE)
    ...
    WHITE_SMOKE = colors.HexColor("#F8F9FA")  # SOB Design System
    c.setFillColor(WHITE_SMOKE)
    c.rect(40, table_y - 2, table_w, 18, fill=1, stroke=0)
    EBONY = colors.HexColor("#0D0A1B")  # SOB Design System
